...
class TestCloudSecurityScanner(unittest.TestCase):
    ...
    def test_iam_users_no_mfa_detected(self):
        ...
        vulnerabilities = self.scanner.report['IAM_Vulnerabilities']
        self.assertTrue(any(v['Resource'] == 'test-user' and v['Issue'] == 'No MFA enabled for user.' for v in vulnerabilities))

    @mock_aws
    def test_ebs_volumes_unencrypted_detected(self):
        """Benchmark: Should detect an unencrypted EBS volume"""
        ec2 = boto3.client('ec2', region_name='us-east-1')
        vol = ec2.create_volume(AvailabilityZone='us-east-1a', Size=10, Encrypted=False)
        self.scanner.scan_ebs_volumes()
        vulnerabilities = self.scanner.report['EBS_Vulnerabilities']
        self.assertTrue(any(v['Resource'] == vol['VolumeId'] and v['Issue'] == 'EBS volume is not encrypted.' for v in vulnerabilities))
    ...
    def test_lambda_functions_unencrypted_env_vars_detected(self):
        ...
        role = iam.create_role(RoleName='lambda-role', AssumeRolePolicyDocument='{}')
        lam.create_function(
        ...
        self.assertTrue(any(v['Resource'] == 'test-lambda' and v['Issue'] == 'Environment variables lacking KMS encryption.' for v in vulnerabilities))
    ...
    def test_api_gateway_lacking_waf_detected(self):
        ...
        resource_id = apigw.get_resources(restApiId=api['id'])['items'][0]['id']
        apigw.create_stage(restApiId=api['id'], deploymentId='dummy', stageName='prod')
        self.scanner.scan_api_gateways()
        vulnerabilities = self.scanner.report['APIGateway_Vulnerabilities']
        self.assertTrue(any('test-api' in v['Resource'] and v['Issue'] == 'API stage lacks WAF protection.' for v in vulnerabilities))
